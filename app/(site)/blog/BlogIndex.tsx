...
async function getPosts(page: number) {
    const start = (page - 1) * POST_LIMIT
    const end = start + POST_LIMIT + 1
    ...
    return {
        ...
        hasNextPage: posts.length > POST_LIMIT,
    }
}
...
export default async function BlogIndex({ page = 1 }: BlogIndexProps) {
    const currentPage = Math.max(1, page)
    const { posts, hasNextPage } = await getPosts(currentPage)
    const hasPreviousPage = currentPage > 1
    const firstArticleNumber = (currentPage - 1) * POST_LIMIT + 1
    const lastArticleNumber = firstArticleNumber + posts.length - 1
    const jsonLd = [
        {
            '@context': 'https://schema.org',
            '@type': 'CollectionPage',
            name: 'Graphene News, Research and Industrial Applications',
            description: 'Follow graphene research, production equipment, batteries, sensors, coatings, aerospace, biomedical research, and industrial graphene applications from USA Graphene.',
            url: currentPage === 1 ? 'https://www.usa-graphene.com/blog/' : `https://www.usa-graphene.com/blog/page/${currentPage}/`,
        ...
        <div className="relative isolate min-h-screen bg-[#070d1a] py-24 sm:py-32 overflow-hidden">
            ...
            <div className="relative mx-auto max-w-7xl px-6 lg:px-8">
                ...
                <div className="mt-10 flex flex-col items-center gap-5">
                    <p className="text-center text-sm font-medium text-[#8b9ab5]">
                        Showing articles {firstArticleNumber}-{lastArticleNumber}.
                    </p>
                    <nav className="flex flex-wrap items-center justify-center gap-3" aria-label="Blog pagination">
                        {hasPreviousPage && (
                            <Link
                                href={pageHref(currentPage - 1)}
                                className="rounded-full border border-white/10 bg-white/[0.04] px-5 py-3 text-sm font-semibold text-white transition-colors hover:border-[#00c8ff]/50 hover:bg-white/[0.08]"
                            >
                                Previous page
                            </Link>
                        )}
                        {hasNextPage && (
                            <Link
                                href={pageHref(currentPage + 1)}
                                className="rounded-full border border-[#00c8ff]/40 bg-[#00c8ff]/10 px-5 py-3 text-sm font-semibold text-[#bfefff] transition-colors hover:border-[#00c8ff] hover:bg-[#00c8ff]/20"
                            >
                                Next page
                            </Link>
                        )}
